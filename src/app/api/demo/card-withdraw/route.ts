...
export async function POST(req: Request) {
  ...
  if (pendingErr || !pendingRow) {
    console.error("[card-withdraw] pending insert failed:", pendingErr);
    return NextResponse.json(
  ...
  const { error: debitErr } = await admin
    ...
    .eq("currency", "USD");
  if (debitErr) {
    console.error("[card-withdraw] debit failed:", debitErr);
  ...
  if (completeErr || !completedRow) {
    console.error("[card-withdraw] complete failed:", completeErr);
    return NextResponse.json(
