...
export interface CardWithdrawFlowProps {
  userId: string;
...
export function CardWithdrawFlow({
  userId,
  initialCards,
  balances,
}: CardWithdrawFlowProps) {
  ...
  const usdBalance = Number(
    balances.find((b) => b.currency === "USD")?.amount ?? 0,
  );
  const amountNum = useMemo(
    () => parseFloat(amount.replace(",", ".")) || 0,
    [amount],
  );
  const overBalance = amountNum > usdBalance;
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/demo/card-withdraw", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        if (err.issues?.length) {
          console.error("[/retirar/tarjeta] validation issues:", err.issues);
        }
        if (err.error === "insufficient_funds") {
          playError();
          toast.error("No tenés suficiente saldo en Dólares");
          setSubmitting(false);
          return;
        }
        throw new Error(err.error ?? "card_withdraw_failed");
      }
  ...
  if (stage === "success" && tx) {
    return (
      <TransactionSuccess
        ...
        subtitle={`−${formatMoney(amountNum, "USD")} a tu tarjeta`}
        tx={tx}
        userId={userId}
      />
    );
  ...
  return (
    <div className="flex min-h-dvh flex-col">
      <header className="pt-safe flex items-center gap-2 px-3 pb-3">
        ...
        <h1 className="text-base font-semibold">Retirar a tarjeta</h1>
      </header>
        ...
        {stage === "select-card" && (
          ...
          >
            <p className="text-sm text-muted-foreground">
              Elegí a qué tarjeta querés enviar tu saldo en USD.
            </p>
            ...
            {cards.length === 0 ? (
              <div className="flex flex-col items-center gap-3 rounded-[var(--radius-card)] bg-surface p-8 text-center ring-1 ring-border">
                ...
                <p className="text-xs text-muted-foreground">
                  Agregá una tarjeta para retirar a ella.
                </p>
              </div>
            ) : (
          ...
          >
            ...
            <div className="flex flex-1 flex-col items-center justify-end pb-8">
              <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
                Dólares · disponible {formatMoney(usdBalance, "USD")}
              </p>
              ...
              {overBalance && (
                <p className="mt-2 text-xs font-medium text-error">
                  No tenés suficiente saldo en Dólares
                </p>
              )}
            </div>
